...
def calculate_psnr(img1: np.ndarray, img2: np.ndarray) -> float:
    ...
    Higher PSNR values indicate greater similarity between images.
    """
...
def compare_images(
    ...
    try:
        ...
        # Calculate PSNR (noise-to-signal ratio is inverse of PSNR)
        psnr_value = calculate_psnr(smaller_img_rgb, resized_img)

        # Calculate noise-to-signal ratio (NSR) as 1/PSNR
        nsr_value = 1.0 / psnr_value if psnr_value > 0 else float("inf")
        ...
        )  # More is better -> 20 is acceptable, 30 is high quality, Above 40 is extremely high quality
        typer.echo(
            f"Noise-to-Signal Ratio (NSR): {nsr_value:.6f}"
